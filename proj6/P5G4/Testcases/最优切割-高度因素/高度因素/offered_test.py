...
optimal_heights = {
    'optHsets': [200,300,400,500,600,700,800,900,1000,2000]
}
...
def run_tests(test_cases, test_case_generator, test_case_name, exe_paths, names):
    global_stats_dir = os.path.join(f'C:/Users/pc/Desktop/test/最优切割/高度因素/{test_case_name}', 'global_stats')
    ...
    for exe_path, name in zip(exe_paths, names):
        ...
        for i, (test_case_file_name, test_case) in enumerate(test_cases):
            ...
            test_case_dir = os.path.join(f'C:/Users/pc/Desktop/test/最优切割/高度因素/{test_case_name}', test_case_file_name, name)
        ...
        all_optimal_heights.append(optimal_height)
    ...
    for name in names:
        ...
        optimal_heights_new = np.linspace(min(optimal_heights['optHsets']), max(optimal_heights['optHsets']), 500)
        spl = make_interp_spline(optimal_heights['optHsets'], times, k=1)
        times_smooth = spl(optimal_heights_new)
        
        plt.plot(optimal_heights_new, times_smooth, label=name)

    plt.xlabel('optH')
    plt.ylabel('Average execution time (seconds)')
    plt.title(f'Average execution time as a function of the optH')
    plt.legend()
    plt.grid(True)
    ...
    for name in names:
        ...
        optimal_heights_new = np.linspace(min(optimal_heights['optHsets']), max(optimal_heights['optHsets']), 500)
        spl = make_interp_spline(optimal_heights['optHsets'], ratios, k=1)
        ratios_smooth = spl(optimal_heights_new)
        
        plt.plot(optimal_heights_new, ratios_smooth, label=name)


    plt.xlabel('optH')
    plt.ylabel('Average approximation ratio')
    plt.title(f'Average approximation ratio as a function of the optH')
    plt.legend()
    plt.grid(True)
...
test_case_directory_paths = ["C:/Users/pc/Desktop/test/最优切割/高度因素/optHsets"]
