...
def run_tests(test_cases, test_case_generator, exe_paths, names):
    for name, exe_path in zip(names, exe_paths):
        ...
        spl = make_interp_spline([len(widths) for widths, _, _ in test_cases], elapsed_times, k=1)
        ...
        spl = make_interp_spline([len(widths) for widths, _, _ in test_cases], approx_ratios, k=1)
